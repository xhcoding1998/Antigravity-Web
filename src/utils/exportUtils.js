 ...
 * 生成 Markdown 内容
 */
export function generateMarkdownContent(messages, title = '对话导出', messageIndices = null) {
  let markdown = `# ${title}\n\n`;
...
export async function downloadDOMAsPDF(element, filename) {
  try {
    ...
    // 后续页
    while (heightLeft > 0) {
      position = heightLeft - imgHeight; // 这里的计算是: 当前剩余顶部位置
      // 实际上 jspdf 分页添加图片的逻辑是: 调整 y 坐标为负值，让图片上移
      // position 应该是 -297, -594 ...
      // 修正算法：
      // Page 2: position should be -297

      pdf.addPage();
      // 计算新的偏移量
      // 上一次的 position 是 0（相对于第一页顶端）
      // 下一页我们要显示的是原本 297mm 处的内容，让它出现在新页面的 0mm 处
      // 所以 y = -297
      // 我们可以维护一个累加的 offset

      // 重置 position 逻辑，直接用 page index
